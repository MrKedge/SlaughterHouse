<div id="defaultModal" tabindex="-1" aria-hidden="true"
    ...
    <div class="relative p-4 w-full max-w-2xl h-full md:h-auto">
        <!-- Modal content -->
        <div class="relative p-4 bg-white rounded-lg shadow 0 sm:p-5">
            <!-- Modal header -->
            ...
            <form action="{{ route('inspector.postmortem.good', ['id' => $animal->id]) }}" method="post">
                ...
                <div class="grid gap-4 mb-4 sm:grid-cols-2">
                    <div>
                        <label for="name" class="block mb-2 text-sm font-medium text-gray-900 ">Carcass</label>
                        <p type="text" name="name" id="name"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 "
                            placeholder="Type product name" required="">{{ $animal->type }}</p>
                    </div>
                    <div>
                        <label for="brand" class="block mb-2 text-sm font-medium text-gray-900 ">Result</label>
                        <p type="text" name="goodStatus" id="brand"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 "
                            placeholder="Product brand" required="">Good meat</p>
                    </div>
                    <div>
                        <label for="weight" class="block mb-2 text-sm font-medium text-gray-900 ">Carcass
                            Weight</label>
                        <input type="number" name="postWeight" id="kilo" min="1"
                            max="{{ $animal->live_weight }}" oninput="limitInputValue(this)"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 "
                            placeholder="Kg." required="">
                    </div>
                    <div>
                        <label for="inspected by" class="block mb-2 text-sm font-medium text-gray-900 ">Inspected
                            By</label>
                        <input type="text" name="inspectedBy" id="brand"
                            value="{{ auth()->user()->first_name }} {{ auth()->user()->last_name }}" readonly
                            class="cursor-not-allowed bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 "
                            required>
                ...
                <button type="submit"
                    class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 focus:outline-none ">Continue</button>
            </form>
        </div>
    </div>
</div>
        ...
        <div class="relative p-4 bg-white rounded-lg shadow  sm:p-5">
            ...
            <form action="{{ route('inspector.condemn.animal', ['id' => $animal->id]) }}" method="post">
                ...
                <div class="grid gap-4 mb-4 sm:grid-cols-2">
                    ...
                    <div>
                        <label for="condemn weight" class="block mb-2 text-sm font-medium text-gray-900 ">Condemn
                            Weight</label>
                        <input type="number" name="condemnWeights" id="condemnWeight" oninput="limitInputValue(this)"
                            min="1"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5  "
                            placeholder="Kg." required="">
                    </div>
                    ...
                    <div class="sm:col-span-2">
                        <label for="description" class="block mb-2 text-sm font-medium text-gray-900 ">Remarks</label>
                        <textarea id="description" rows="4"
                ...
                @if ($animal->condemnCarcasses->isEmpty())
                    <button type="submit"
                        class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 focus:outline-none ">Save
                    </button>
                @else
                    <button type="submit"
